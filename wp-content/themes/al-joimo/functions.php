...
function joimo_kombucha_scripts()
{
	...
	wp_enqueue_script('my-products', get_template_directory_uri() . '/js/my-products.js', array(), _S_VERSION, true);
	...
	wp_enqueue_script('fontawesome-js', get_template_directory_uri() . '/js/all.js', array('jquery'), '6.14.0', true);
	...
	wp_enqueue_style( 'owlcarousel-style', get_template_directory_uri() . '/inc/owl/owl.carousel.min.css' );
	wp_enqueue_style( 'owlcarousel-style', get_template_directory_uri() . '/inc/owl/owl.theme.default.min' );
...
// NEW FUNCTION view price with right place

function wc_ninja_custom_variable_price($price, $product) {
    // Main Price
    $regular_prices = array($product->get_variation_regular_price('', true), $product->get_variation_regular_price('max', true));
    ...
    // Sale Price
    $sale_prices = array($product->get_variation_sale_price('min', true), $product->get_variation_sale_price('max', true));
    sort($sale_prices);
    $sale_price = $sale_prices[0] !== $sale_prices[1] ? sprintf(__('From %1$s', 'woocommerce'), wc_price($sale_prices[0])) : wc_price($sale_prices[0]);

    // Check if sale price exists and is different from regular price
    if ($sale_prices[0] !== $regular_prices[0]) {
        $price = sprintf(__('From %2$s To %1$s', 'woocommerce'), wc_price($regular_prices[0]), wc_price($sale_prices[0]));
    } else {
        $price = $regular_price;
    }
